...
export default class UID {
    ...
    constructor(hoyo_type: HoyoType_T, data: UIDResponse) {
        this.data = data;
        ...
        switch(hoyo_type) {
            case HoyoType.GI: {
                ...
                this.avatars = giData.avatarInfoList.map((avatar) => getCharacter(this.hoyo_type, String(avatar.avatarId)));
                this.nickname = giData.playerInfo.nickname;
                break;
            }
            case HoyoType.HSR: {
                const hsrData = data as HSRUIDResponse
                this.avatars = hsrData.detailInfo.avatarDetailList.map((avatar) => getCharacter(this.hoyo_type, String(avatar.avatarId)));
                this.nickname = hsrData.detailInfo.nickname;
                break;
            }
            case HoyoType.ZZZ: {
                const zzzData = data as ZZZUIDResponse;
                this.avatars = zzzData.PlayerInfo.ShowcaseDetail.AvatarList.map((avatar) => getCharacter(this.hoyo_type, String(avatar.Id)));
